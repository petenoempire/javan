    ...
    <div className="relative h-[100dvh] w-full snap-start overflow-hidden bg-black">
      ...
      <div className="absolute bottom-32 right-3 z-10 flex flex-col items-center gap-5 text-white">
        ...
        <button onClick={toggleLike} className="flex flex-col items-center gap-1 active:scale-90" aria-label="Like">
          <div className={`glass-strong flex h-14 w-14 items-center justify-center rounded-full border border-white/10 shadow-glow-sm ${liked ? "heart-burst" : ""}`}>
            <Heart className={`h-8 w-8 ${liked ? "fill-rose text-rose drop-shadow-[0_0_8px_rgba(255,45,85,0.6)]" : "text-white"}`} />
          </div>
          <span className="text-xs font-black drop-shadow-lg">{fmt(likeCount)}</span>
        </button>
        <button onClick={onComment} className="flex flex-col items-center gap-1 active:scale-90" aria-label="Comment">
          <div className="glass-strong flex h-14 w-14 items-center justify-center rounded-full border border-white/10 shadow-glow-sm">
            <MessageCircle className="h-8 w-8 text-white" />
          </div>
          <span className="text-xs font-black drop-shadow-lg">{fmt(video.comment_count)}</span>
        </button>
        ...
        <button onClick={() => { setSaved((s) => !s); toast.success(saved ? "Removed from saved" : "Saved"); }} className="flex flex-col items-center gap-1 active:scale-90" aria-label="Save">
          <div className="glass-strong flex h-14 w-14 items-center justify-center rounded-full border border-white/10 shadow-glow-sm">
            <Bookmark className={`h-7 w-7 ${saved ? "fill-amber-400 text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.6)]" : "text-white"}`} />
          </div>
        </button>
