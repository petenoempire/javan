    ...
    <>
      ...
      <DesktopLayout>
        <div className="max-w-5xl mx-auto py-10">
          <div className="relative h-48 rounded-[2.5rem] overflow-hidden bg-gradient-to-r from-fuchsia-600 to-rose-600 shadow-glow mb-20">
             <div className="absolute -bottom-16 left-12 h-32 w-32 rounded-full bg-gradient-to-br from-rose-500 to-purple-600 border-4 border-[#020210] shadow-glow" />
          </div>
          ...
          <div className="px-12 flex items-start justify-between mb-10">
             <div className="flex-1">
                ...
                <p className="text-xl text-white/50 mt-1">{profile.display_name}</p>
                {profile.bio && <p className="mt-6 text-lg text-white/80 max-w-2xl leading-relaxed">{profile.bio}</p>}
                <div className="flex gap-8 mt-8">
                   <div className="text-center">
                      <p className="text-2xl font-black text-white">{followers.toLocaleString()}</p>
                      <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Followers</p>
                   </div>
                   <div className="text-center">
                      <p className="text-2xl font-black text-white">{following.toLocaleString()}</p>
                      <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Following</p>
                   </div>
                   <div className="text-center">
                      <p className="text-2xl font-black text-white">{posts.length.toLocaleString()}</p>
                      <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Posts</p>
                   </div>
                </div>
             </div>
             ...
             {!isSelf && (
                <div className="flex flex-col gap-3 w-48">
                   <Button 
                      ...
                      className={`rounded-2xl py-6 font-black ${isFollowing ? "bg-white/10 text-white" : "bg-gradient-primary text-white shadow-glow"}`}
                   >
                      {isFollowing ? <><UserCheck className="h-5 w-5 mr-2" /> Following</> : <><UserPlus className="h-5 w-5 mr-2" /> Follow</>}
                   </Button>
                   <Button variant="outline" className="rounded-2xl py-6 border-white/10 hover:bg-white/5 font-black">
          ...
          <div className="px-12 grid grid-cols-3 gap-6 pb-20">
             {posts.map(p => (
                <Link key={p.id} to="/posts/$id" params={{ id: p.id }} className="aspect-[9/16] rounded-3xl glass border border-white/10 overflow-hidden relative group">
                   {p.video_url ? (
                      <video src={p.video_url} className="h-full w-full object-cover" />
                   ) : (
      ...
      <MobileShell>
        <div className="relative">
          ...
          <div className="px-5 pb-6">
            ...
            <h2 className="sr-only">Profile stats</h2>
        ...
        <div className="px-5 pb-8">
          <h2 className="sr-only">{tab === "music" && isArtist ? "Music tracks" : "Content feed"}</h2>
