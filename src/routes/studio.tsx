           ...
           <div className="lg:col-span-8 space-y-8">
              <div className="glass p-8 rounded-[2.5rem] border border-white/5">
                 <div className="flex items-center justify-between mb-6">
                    ...
                    <Link to="/studio/$section" params={{ section: "analytics" }} className="text-xs text-cyan-400 font-bold hover:underline">View detailed report</Link>
                 </div>
                 <div className="grid grid-cols-3 gap-6">
                    <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                       <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-2">Views</p>
                       <p className="text-3xl font-black text-white">{stats?.views?.toLocaleString() ?? 0}</p>
                    </div>
                    <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                       <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-2">Followers</p>
                       <p className="text-3xl font-black text-cyan-400">{stats?.followers?.toLocaleString() ?? 0}</p>
                    </div>
                    <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                       <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-2">Likes</p>
                       <p className="text-3xl font-black text-rose-500">{stats?.likes?.toLocaleString() ?? 0}</p>
                    </div>
                 </div>
              </div>
                 ...
                 <div className="space-y-4">
                    {["Hooks that hold", "Lighting like a pro", "Caption strategy"].map(title => (
                       <div key={title} className="flex items-center gap-3 group cursor-pointer">
                          ...
                          <span className="text-sm font-medium group-hover:text-cyan-400 transition-colors">{title}</span>
                       </div>
                    ))}
                 </div>
              </div>
